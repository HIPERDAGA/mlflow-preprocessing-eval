# -*- coding: utf-8 -*-
"""
Evalúa CLAHE bajo una condición (p. ej., 'fog') usando métricas NIQE/PIQE/BRISQUE
...
Uso (Colab):
    ...
    !python eval_clahe.py --data "/content/drive/MyDrive/datasets/fog" \
        --condition fog --clip 2.0 --tiles 8 \
        --experiment fog-preprocessing-evaluation-CLAHE
"""
...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=float,
        default=2.0,
        help="clipLimit de CLAHE",
    )
    ap.add_argument(
        "--tiles",
        type=int,
        default=8,
        help="tileGridSize (entero, se usa NxN)",
    )
    ap.add_argument(
        "--experiment",
    ...
    # Esto es opcional, por si ya tenías --method (no se usa, pero no molesta)
    ap.add_argument(
        "--method",
        default="clahe",
        help="Método (no usado, solo para compatibilidad)",
    )
...
def main():
    ...
    with mlflow.start_run(run_name=f"CLAHE_{args.condition}"):
        ...
        with open("artifacts/summary_clahe.txt", "w", encoding="utf-8") as f:
            f.write(summary)
        mlflow.log_artifact("artifacts/summary_clahe.txt")

        print("✅ Evaluación CLAHE completada.")
        print(summary)
